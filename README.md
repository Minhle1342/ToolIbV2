...
* File này sẽ tự động chạy script PowerShell **`run.ps1`**, kích hoạt môi trường ảo `.venv` (nếu có), tự động mở trình duyệt hiển thị ứng dụng tại địa chỉ `http://localhost:5000` và khởi chạy server Flask.
...
python app.py
```
Sau đó, truy cập vào địa chỉ: `http://localhost:5000` (hoặc `http://<IP_MAY_CHỦ>:5000` nếu truy cập từ máy khác trong mạng).
