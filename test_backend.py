...
def setup_dummy_data(images_dir):
    classes_file = os.path.join(images_dir, "classes.txt")
    os.makedirs(images_dir, exist_ok=True)

    for i in range(1, 6):
        with open(os.path.join(images_dir, f"img_{i}.jpg"), 'w', encoding='utf-8') as f:
...
def run_backend_smoke_test():
    with tempfile.TemporaryDirectory(prefix='toolib_backend_test_') as temp_dir:
        test_dir = os.path.join(temp_dir, 'test_data')
        images_dir = os.path.join(test_dir, 'images')
        db_path = os.path.join(temp_dir, 'test_backend.db')
        setup_dummy_data(images_dir)
        ...
        with app.app_context():
            ...
            export_path = os.path.abspath("exported_dataset")
